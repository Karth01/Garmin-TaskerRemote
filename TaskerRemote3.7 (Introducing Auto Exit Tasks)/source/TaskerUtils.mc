...
class TaskerUtils {
    ...
    // Cached task data to avoid repeated property lookups (expanded to 19 tasks)
    ...
    static function initializeTaskCache() as Void {
        ...
        _cachedTasks = new Array<String>[20]; // Index 0 unused, 1-19 align with task numbers
    ...
    // Trigger a task by its index (1-19)
    ...
    static function triggerTask(taskIndex as Lang.Number) as Lang.Boolean {
        return triggerTaskWithExit(taskIndex, false);
    }

    // Trigger a task by its index (1-19) with optional auto-exit
    // Returns true if task was triggered successfully, false otherwise
    static function triggerTaskWithExit(taskIndex as Lang.Number, shouldExit as Lang.Boolean) as Lang.Boolean {
        initializeTaskCache();
        
        if (taskIndex < 1 || taskIndex > 19 || _cachedTasks == null) {
            return false;
        }
        ...
        if (taskName.length() > 0) {
            ...
            // If shouldExit is true, exit the app after triggering
            if (shouldExit) {
                performExit();
            }
    ...
    // Check if a task index is an auto-exit task (2-6)
    static function isAutoExitTask(taskIndex as Lang.Number) as Lang.Boolean {
        return taskIndex >= 2 && taskIndex <= 6;
    }
