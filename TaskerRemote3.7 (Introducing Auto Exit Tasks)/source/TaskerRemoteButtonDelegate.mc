...
class TaskerRemoteButtonDelegate extends WatchUi.BehaviorDelegate {
    ...
    function handleBackButton() as Lang.Boolean {
        ...
        if (useNewBehavior) {
            ...
            if (TaskerUtils.isTaskConfigured(19)) {
    ...
    // Handle UP button press -> Task 17 (single press only)
    function onNextPage() as Lang.Boolean {
        return handleButtonPress(BUTTON_UP);
    }

    // Handle DOWN button press -> Task 16 (single press only)
    function onPreviousPage() as Lang.Boolean {
        return handleButtonPress(BUTTON_DOWN);
    }

    // Handle SELECT button press -> Task 18 (single press only)
    function onSelect() as Lang.Boolean {
        return handleButtonPress(BUTTON_SELECT);
    }

    // Handle MENU button press -> Same as UP button (Task 17)
    function onMenu() as Lang.Boolean {
        return handleButtonPress(BUTTON_UP);
    }

    // Handle BACK button press -> Task 19 (single) / Menu/Exit (double press)
    ...
    function getSingleTaskIndex(buttonId as Lang.Number) as Lang.Number {
        if (buttonId == BUTTON_UP) {
            return 17; // UP button -> task_17
        } else if (buttonId == BUTTON_DOWN) {
            return 16; // DOWN button -> task_16
        } else if (buttonId == BUTTON_SELECT) {
            return 18; // SELECT button -> task_18
        }
